    ...
    with torch.no_grad():
        for data, labels in test_dataloader:
            ...
            for embedding in embeddings:
                ...
                distances = [torch.nn.functional.pairwise_distance(embedding.unsqueeze(0), genuine_embedding.unsqueeze(0)).item() for genuine_embedding in known_genuine_embeddings]
...
if __name__ == '__main__':
    ...
    for fold, (train_idx, val_idx) in enumerate(kfold.split(dataset)):
        ...
        model = TransformerKeystrokeModel(input_dim=data.shape[-1], model_dim=128, num_heads=8, num_layers=6).to(device)
